...
<body>
	...
	<?php
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
		    ...
		    if(empty($_POST['Code'])) {
			$errors[] = 'You forgot to enter the Admin Code.';
	...
	if(empty($errors)) {
		...
		*/
		//==================================================//
		// We use the auto-Incrementation for the admin_no //
		//=================================================//
		$req=$bdd->prepare('INSERT INTO admin(first_name, last_name, password, date_of_birth, gender, mobile, home_tel, email, address) VALUES(:first_name,  :last_name, :password, :date_of_birth, :gender, :mobile, :home_tel, :email, :address)');
		$req->execute(array(
		...
		if($req) { // if the query ran successfully
					header ('location: success_admin.html');
			}
			else {
		...
		echo $email;
		$reponse = $bdd->query("SELECT * FROM admin WHERE email = '$email' ");
